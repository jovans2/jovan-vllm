...
def main(args: argparse.Namespace):
    ...
    def run_to_completion(profile_dir, inputs, out_lent):
        sampling_params = SamplingParams(
            n=args.n,
            temperature=0.0 if args.use_beam_search else 1.0,
            top_p=1.0,
    ...
    print("Warming up...")
    run_to_completion(None, dummy_prompt_token_ids, args.output_len)
    ...
    for in_len in input_lens:
        for batch_sz in batch_sizes:
            ...
            dummy_prompt_token_ids = dummy_prompt_token_ids.tolist()
            latency1 = run_to_completion(None, dummy_prompt_token_ids, 1)
            for _ in tqdm(range(args.num_iters), desc="Profiling iterations"):
                dummy_prompt_token_ids = np.random.randint(10000, size=(batch_sz, in_len))
                dummy_prompt_token_ids = dummy_prompt_token_ids.tolist()
                latency2 = run_to_completion(None, dummy_prompt_token_ids, args.output_len)
                latencies.append((latency2-latency1)/(args.output_len-1))
